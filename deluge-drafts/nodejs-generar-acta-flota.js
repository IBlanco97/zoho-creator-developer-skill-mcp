...
module.exports = async function(context, basicIO) {
  ...
  async function uploadFileToField(token, reportName, recordId, fieldName, fileBuffer, filename) {
    var boundary = '----ActaBoundary' + recordId + 'XZ7';
    var CRLF     = '\r\n';
  ...
  async function buildPDF(d) {
    var pdfLib = require('./pdf-lib-bundle');
  ...
  try {
    ...
    console.log('Procesando asignacion #' + assignId);
    ...
    var uploadResp = await uploadFileToField(token, 'Historial_de_Asignaciones', assignId, 'Documento_de_respaldo_de_asignaci_n', pdfBuffer, filename);
    var uploadBody = uploadResp.body.toString('utf8');
    console.log('Upload status=' + uploadResp.status + ' body=' + uploadBody.substring(0, 120));
    ...
    console.error('GenerarActaFlota: ' + (err.message || String(err)));
